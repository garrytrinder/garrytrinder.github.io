...
- Get the latest available Kubernetes version in your region
  - `version=$(az aks get-versions -l uksouth --query 'orchestrators[-1].orchestratorVersion' -o tsv)`
  - Uses JMESPath to pull back the version using text output from the AKS service
  - `echo $version` = `1.19.6` - Feb 2021
- Create Resource Group
  - `az group create -l uksouth -n rg-aksdevopslab-dev`
- Create AKS
  - `az aks create --resource-group rg-aksdevopslab-dev --name aks-devopslab-dev --enable-addons monitoring --kubernetes-version $version --generate-ssh-keys -l uksouth`
  - Requires unique name for the cluster
  - We pass in the latest version as a variable `$version`
  - Log Analytics are turned on by stating `monitoring` as an addon
  - SSH keys are generated and stored in `.ssh` for authentication
    - Warning message: `SSH key files '/Users/garrytrinder/.ssh/id_rsa' and '/Users/garrytrinder/.ssh/id_rsa.pub' have been generated under ~/.ssh to allow SSH access to the VM. If using machines without permanent storage like Azure Cloud Shell without an attached file share, back up your keys to a safe location`
- Deploy to Azure Container Registry
  - `az acr create -g rg-aksdevopslab-dev -n acraksdevopslabdev --sku Standard -l uksouth`


```sh
# Get latest version of AKS for the target region
version=$(az aks get-versions -l uksouth --query 'orchestrators[-1].orchestratorVersion' -o tsv)

# Create resource group
az group create -l uksouth -n rg-aksdevopslab-dev

# Create Azure Kubernetes Service cluster enabling Log Analytics and generating SSH keys
az aks create --resource-group rg-aksdevopslab-dev --name aks-devopslab-dev --enable-addons monitoring --kubernetes-version $version --generate-ssh-keys -l uksouth
...
CLIENT_ID=$(az aks show --resource-group rg-aksdevopslab-dev --name aks-devopslab-dev --query "identityProfile.kubeletidentity.clientId" --output tsv)

# Get the Resource ID of the Azure Container Registry
ACR_ID=$(az acr show --name acraksdevopslabdev --resource-group rg-aksdevopslab-dev --query "id" --output tsv)

# Grant the AKS Cluster Pull rights to the Azure Container Registry
az role assignment create --assignee $CLIENT_ID --role acrpull --scope $ACR_ID

# Create Azure SQL Server
az sql server create -l uksouth -g rg-aksdevopslab-dev -n sql-aksdevopslab-dev -u sqladmin -p P@ssw0rd1

# Create Database
az sql db create -g rg-aksdevopslab-dev -s sql-aksdevopslab-dev -n sqldb-aksdevopslab-dev --service-objective S0
```